...
global $wpdb;
  ...
  <div class="csmr_menu_item"><a href="?page=ele&action=newcontract_form" title="Укласти договір"><img src="<?php echo $icons_path ?>new_contract.png"><br />Укласти договір</a></div>
  <div class="csmr_menu_item"><a href="?page=ele&action=addplan_form"><img src="<?php echo $icons_path ?>upload_plan.png"><br />Подати план</a></div>
  <div class="csmr_menu_item"><a href="?page=ele&action=addcorr_form"><img src="<?php echo $icons_path ?>correction_plan.png"><br>Подати коригування</a></div>
  <div class="csmr_menu_item"><a href="?page=ele&action=addfact_form"><img src="<?php echo $icons_path ?>upload_fact.png"><br>Подати факт</a></div>
  <div class="csmr_menu_item"><a href="?page=ele&action=getinvoice_form"><img src="<?php echo $icons_path ?>make_invoice.png"><br>Сформувати рахунок</a></div>
...
case "newcontract_form":
  ?>
    <div class="wrap">
      ...
      <hr>
       <form id="add_contract_form" name="add_contract_form" action="?page=ele&action=newcontract_processing" method="post" enctype="multipart/form-data">
         <input type="hidden" id="contract_type" value="ele">
...
$PCCN = $wpdb->get_var('SELECT provider_contract_number FROM consumers_contracts WHERE contract_type="ele" ORDER BY internal_id DESC LIMIT 1');
  if (!isset($PCCN)) {
...
//    DD($PCN);
  ...
  $last_contract_internal_id = $wpdb->get_var('SELECT * FROM consumers_contracts WHERE contract_type="ele" ORDER BY internal_id DESC LIMIT 1');
...
//check allowed time for plan start date
      ...
      $query = 'SELECT * FROM consumers_contracts WHERE contract_type="ele" AND consumer_id='.get_current_user_id().' AND contract_end_date > CURDATE()+2';
      ...
      if (!empty($contracts)) {
        ...
        <div class="wrap">
          ...
          <form name="add_plan" id="add_plan" action="?page=ele&action=addplan_processing" method="post" enctype="multipart/form-data">
        ...
        $error_msg=' <div class="error"><br>Ви не маєте активних договорів, по яким можна було б подати планове споживання.<br> Спочатку додайте договір у розділі <a href="?page=ele&action=newcontract_form">"Укласти договір"</a><br><br></div>';
        echo $error_msg;
      }
...
case "addplan_processing" :
  ...
  if ($addplan_res == 1) {
    echo "Планове споживання додано до контракту.<br>Очікуйте на підтвердження від менеджера.<br><br><a href='?page=ele&action=addplan_form'>Додати план до іншого контракту</a> | <a href='?page=ele'>Газопостачання</a> | <a href='?page=energy'>Електроенергія</a>";
    ...
    $headers .= "MIME-Version: 1.0\r\n";
    $headers .= "Content-Type: text/html; charset=UTF-8\r\n";
    mail('cms@energogazrezerv.com.ua',mb_encode_mimeheader(__('New plan in CMS by number:','egr-consumer').$icn." | ".__('from','egr-consumer').": ".$firm_name), $msg, $headers);
...
case "addcorr_form":
  ...
  $q = 'SELECT internal_id, consumer_contract_number,contract_end_date FROM consumers_contracts WHERE contract_type="ele" AND contract_end_date > CURDATE()  and consumer_id='.get_current_user_id().' ORDER BY contract_end_date DESC';
...
<h3>Форма подання коригування</h3>
  <form id="corr_form" action="?page=ele&action=addcorr_processing" method="post" enctype="multipart/form-data">
...
case "addcorr_processing":
  ...
  if ($addcorr_res == 1) {
    echo '<div class="success">Коригування додано .<br>Очікуйте на підтвердження від менеджера.<br></div><br><a href="?page=ele&action=addcorr_form">Додати інше коригування</a> | <a href="?page=ele">Газопостачання</a> | <a href="?page=energy">Електроенергія</a>';
  ...
  <div class="wrap">
    <div class="contracts_hdr">Введення фактичного споживання</div>
      <div class="addfact_form">
        <form id="fact_form" action="?page=ele&action=addfact_processing" method="post" enctype="multipart/form-data">
        Оберіть контракт: <select id="fact_contract_id" name="fact_contract_id" required><?php echo $select_opt; ?></select></label></br>
        Введіть об'єм: <input type="number" id="fact_corr_capacity" name="fact_corr_capacity" step="0.00001"  min="0.00001" max="999999999.99" title="будь ласка введіть обєм у формати хххxxx,ххххх" size="30" required disabled><br>
...
case "addfact_processing":
  ...
  if ($addfact_res ==1) {
    echo '<div class="success">Фактичне споживання додано .<br>Очікуйте на підтвердження від менеджера.<br></div><br><a href="?page=ele&action=addfact_form">Додати ще факт</a> | <a href="?page=ele">Газопостачання</a> | <a href="?page=energy">Електроенергія</a>';
    $ctr_id = $wpdb->get_var("SELECT contract_id FROM consumer_contract_plans WHERE plan_id=".$_POST['addcorr_plan_id']); // get contract id by current plan_id;
    $ctr_name = $wpdb->get_var("SELECT provider_contract_number FROM consumers_contracts WHERE internal_id=".$ctr_id);
    // send email notification to managers
    $firm_name = get_usermeta(get_current_user_id(),'firm_name');
...
case "getinvoice_form":
    ...
    ?>
    <form action="?page=ele&action=invoice_processing" method="POST" enctype="multipart/form-data" id="inv_form">
      <input id="unit_type" type="hidden" value="ele">
...
case "invoice_processing":
  ...
  if ($_POST['inv_valtype'] =='price') {$amount = "Cумма рахунку: <b>".$_POST['inv_amount']." грн.</b>"; }
    ...
    $amount = "Об'єм споживання ел.енергії: <b>".$_POST['inv_amount']." кВт.</b>";
  }
  $firm_name = get_usermeta(get_current_user_id(),'firm_name');
  ...
  if (mail('cms@energogazrezerv.com.ua',mb_encode_mimeheader(__('Request for invoice', 'egr-consumer').$firm_name), $msg, $headers)) {
    echo '<div class="success">Запит відправлено.<br>Найближчим часом рахунок надійде вам на електронну скриньку.<br></div><br><a href="?page=ele&action=getinvoice_form">Додати інший запит</a> | <a href="?page=ele">Газопостачання</a> | <a href="?page=energy">Електроенергія</a>';
  } else {
